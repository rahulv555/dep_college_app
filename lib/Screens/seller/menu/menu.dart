...
class _OutletMenuSellerState extends State<OutletMenuSeller> {
  ...
  void _editMenuItem(BuildContext ctx, String category, int i) {
    ...
    showModalBottomSheet(
      ...
      context: ctx,
      builder: (bCtx) {
        return GestureDetector(onTap: () {}, behavior: HitTestBehavior.opaque, child: EditItem(widget._menu[category]![i], category, i, widget._menu.keys, _editItem));
      },
    );
  }
  ...
  void _editItem(String category, int i, String name, String cost, String _originalCategory) {
    ...
    FirebaseFirestore.instance.collection('outlets').doc(widget._selectedOutlet.toString()).collection('menu').doc(category).update({'items': items}).then((value) {
      FirebaseFirestore.instance.collection('outlets').doc(widget._selectedOutlet.toString()).collection('menu').doc(_originalCategory).update({'items': original_items}).then((value) {
        setState(() {
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Column(
                                        crossAxisAlignment: CrossAxisAlignment.start,
                                        children: [
                                          Container(child: Text(widget._menu[category]![i].name, style: TextStyle(fontSize: 20), textAlign: TextAlign.start)),
                                          Container(child: Text('Rs.' + widget._menu[category]![i].price.toString(), textAlign: TextAlign.start, style: TextStyle(fontSize: 20))),
                                        ],
                                      ),
                                      Container(
                                        child: Column(
              ...
              child: Column(
                children: [
                  Row(
                    ...
                    children: [Container(child: Text('MENU', style: TextStyle(fontSize: 40, fontWeight: FontWeight.bold)), alignment: Alignment.center)],
                  ),
